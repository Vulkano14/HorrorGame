...
public class DoubleDoorOpenScript : MonoBehaviour
{
    ...
    void Update()
    {
        if (permamenlyBlocked){
            ...
            transform.GetChild(1).eulerAngles = new Vector3(transform.GetChild(1).eulerAngles.x, Mathf.LerpAngle(currentRotationAngleSecond, defaultRotationAngleSecond + (open ? -FirstChildrenOpenAngle : 0), openTime), transform.GetChild(1).eulerAngles.z);
